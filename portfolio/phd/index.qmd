...
categories: [CMR, pharmacological stress, heart transplant, thesis]
author: 
    name: G. Bisaccia
...
![“Making Space”, sculpture by Sir Antony Gormley, Magdi Yacoub Institute, Harefield Hospital © Adobe Stock](images/harefield.png)
...
source("scripts/db.r")
source("scripts/stressDB.r")
source("scripts/angio.r")
source("scripts/safety.r")
source("scripts/main_caller.R")
...
data %>%
    select(c(cav, gender.factor, cause_2.factor, diabetes_mellitus, hypertension, dyslipidemia, smoking, atrial_fibrillation, ckd)) %>%
    tbl_summary(missing = "no", by = cav) %>%
    add_p() %>%
    add_overall() %>%
    as_gt()
...
data %>%
    select(c(bsa, lvedvi_cmr, lvesvi_cmr, lvef_cmr, lvci_cmr, rvedvi_cmr, rvesvi_cmr, rvef_cmr, rvci_cmr, mapselat_cmr, tapse_cmr, lax_gls_cmr, lax_grs_cmr, la_ef_cmr, hypokinesia_cmr, dyskinesia_cmr, akinesia_cmr, lge_cmr, lge_mass_per_cmr, cav)) %>%
...
```{r safety}
require(echarts4r)
unev <- sum(data$stressor == 1) + sum(data$stressor_v2 == 1, na.rm = T) - sum(
    death, mi_stress, vtvf, hosp_stress, asystole, sinuspause,
    av_block, afib, chest_pain, hypotension, dyspnea, nausea,
    headache, allergic, extravas, thrombophleb
)
safety <- data.frame(
    ae = c(
        "Death", "MI", "Ventricular arrhythmias", "Hospitalisation",
...
stress$cav <- data$cav
stress %>%
    select(-c(age, bsa, stressor, lvef, gls, nosymptom, time_from_tx, time_angio_tx, time_cav_cmr, mace_time, cav_angio, cav_ctca, cav_grade, rejection, death, hosp, mace, tx_old, rmbf, gender, weight, dsa, dsa_fup, mbf, mpr, ischemic_burden)) %>%
    mutate(
...
![Net benefit](images/dca-netben.png)

![Net reduction in unnecessary angiography](images/dca-netred.png)
...
data.frame(
    ...
    e_theme("vintage") %>%
    e_axis_labels(x = "Strategy", y = "Threshold probability")
```
