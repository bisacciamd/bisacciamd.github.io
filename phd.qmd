---
title: "Value of Quantitative Stress Perfusion CMR in the Long-Term Assessment of Cardiac Transplantation Recipients"
subtitle: PhD thesis, 2024
description: "Supervisors: Prof. Sabina Gallina MD, Prof. Chiara Bucciarelli-Ducci, MD, PhD, Prof. Fabrizio Ricci, MD, PhD, MSc"
categories: [CMR, pharmacological stress, heart transplant]
author: 
    name: G. Bisaccia
    degrees: MD, PhD candidate
...
```{r chart-theme}
library(echarts4r)
e_theme_register("vintage")
```


## 

![“Making Space”, sculpture by Sir Antony Gormley, Magdi Yacoub Institute, Harefield Hospital © Adobe Stock](images/phd/harefield.png)
...
library(echarts4r)
df <- data.frame(
  year = as.character(c(2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022)),
  rate = c(0.80, 0.80, 0.90, 0.90, 0.90, 1, 1, 1, 1.50, 1.30, 1.40, 1.50)
)

df %>% e_charts(year) %>% e_bar(rate) %>% e_theme("vintage") %>% e_legend(FALSE) %>% e_x_axis(name = "\nYear", nameLocation = "middle") %>% 
e_y_axis(name = "Rate per million population\n\n", nameLocation = "middle")

```

Rate of heart transplants worldwide, 2011-2022 ([Global Observatory on Donation and Tranplantation](https://www.transplant-observatory.org), adapted).
...
::: {#fig-htxcompl}
![](images/phd/htx-compl.png)

Main complications following cardiac transplantation.
:::
...
Typical atherosclerosis and allograft vasculopathy. (Adapted from Avery 2003 @RN9)
:::
...
::: {#fig-ishlt}
![](images/phd/ishlt.png)

CAV grading according to the ISHLT.
:::
...
data <- data %>% mutate(
  smoking=case_when(
  data$smoking == 1 | is.na(data$smoking) ~ "Never",
  data$smoking == 2 ~ "Current",
  data$smoking == 3 ~ "Former"),
  
  atrial_fibrillation=case_when(
    data$atrial_fibrillation == 1 ~ "None",
    data$atrial_fibrillation == 2 ~ "Paroxysmal",
    data$atrial_fibrillation == 3 ~ "Persistent or permanent"
  ))

label(data$smoking) <- "Smoker"
label(data$atrial_fibrillation) <- "Atrial fibrillation"
label(data$cause_2.factor) <- "Reason for transplantation"
label(data$gender.factor) <- "Sex"

data %>% select(c(cav, gender.factor, cause_2.factor, diabetes_mellitus, hypertension, dyslipidemia, smoking, atrial_fibrillation, ckd)) %>% tbl_summary(missing = "no", by = cav) %>% add_p() %>% add_overall() %>% as_gt()
...
data %>% select(c(bsa, lvedvi_cmr, lvesvi_cmr, lvef_cmr, lvci_cmr, rvedvi_cmr, rvesvi_cmr, rvef_cmr, rvci_cmr, mapselat_cmr, tapse_cmr, lax_gls_cmr,lax_grs_cmr,la_ef_cmr,hypokinesia_cmr,dyskinesia_cmr, akinesia_cmr,lge_cmr,lge_mass_per_cmr, cav)) %>% tbl_summary(by = cav, missing = "no") %>% add_overall %>% add_p() %>% as_gt()
...
Adenosine dosage of 140 µg/kg/min was utilized in the majority of studies (n = 37, 62%) while 210 µg/kg/min was reached in 8 patients (13%). During adenosine infusion, the haemodynamic response was highly variable (@fig-cmr-feas). Heart rate increased from a mean ± SD of 82 ± 14 to 91 ± 18 beats per minute, while systolic blood pressure decreased from 132 ± 16 to 124 ± 14 mm Hg. Overall, 50.1% patients reported chest tightness or chest pain during infusion, 22.4% reported shortness of breath, 17.2% did not experience any symptoms and 10.3% had hot flush. Haemodynamic and symptom response were not dependent on adenosine dosage subgroup (all p-values \> 0.5). Stress CMR findings are detailed in @tbl-stress.

::: {#fig-cmr-feas}
```{r feasibility}
library(echarts4r)
chest <- round((sum(data$stress_symptoms___2)+sum(data$stress_symptoms___2))/sample*100, digits = 1)
nosymp <- round(sum(data$stress_symptoms___1)/sample*100, digits = 1)
short <- round(sum(data$stress_symptoms___3)/sample*100, digits = 1)
hot <- round(sum(data$stress_symptoms___5)/sample*100, digits = 1)

feasibility <- data.frame(
    symp=c("Chest tightness or pain", "No symptoms", "Shortness of breath", "Hot flush"),
    occ=c(chest, nosymp, short, hot)
)
...
feasibility[feasibility$occ>0,] %>% 
  ...
  e_legend(FALSE) %>% e_theme("vintage")
  # |> e_title("Donut chart")
```

Haemodynamic and symptom response to adenosine.
:::

#### Safety

No life-threatening adverse events, nor brief or prolonged atrioventricular block, or major arrythmias occurred during or immediately after adenosine infusion, with one patient having self-reversed sinus pauses. Mild, transient adverse effects were registered: three patients (4.5%) complained of headache, and another reported nausea. @fig-cmr-safe presents safety findings.

::: {#fig-cmr-safe}
```{r safety}
require(echarts4r)
unev <- sum(data$stressor==1)+sum(data$stressor_v2==1, na.rm = T)-sum(death, mi_stress, vtvf, hosp_stress, asystole, sinuspause,
                        av_block, afib, chest_pain, hypotension, dyspnea, nausea,
                        headache, allergic, extravas, thrombophleb)
safety <- data.frame(
    ae= c("Death", "MI", "Ventricular arrhythmias", "Hospitalisation",
                        "Asystole", "Sinus pauses", "AV block", "Atrial fibrillation",
                        "Chest pain", "Hypotension", "Dyspnoea", "Nausea", "Headache",
                        "Allergic reaction", "Extravasation", "Thrombophlebitis", "Uneventful stress CMR"),
    occ=c(death, mi_stress, vtvf, hosp_stress, asystole, sinuspause,
                        av_block, afib, chest_pain, hypotension, dyspnea, nausea,
                        headache, allergic, extravas, thrombophleb, unev))

safety$ae <- paste0(safety$ae, " (", (round(safety$occ/sum(safety$occ)*100, digits = 1)), "%)")
...
Safety of stress perfusion CMR with adenosine after heart transplant.
:::
...
::: {#tbl-stress}
```{r}
stress <- stress %>% mutate(ado_dose = case_when(
  stress$ado_dose==1 ~ "140",
  stress$ado_dose==2 ~ "180",
  stress$ado_dose==3 ~ "210"))

label(stress$ado_dose) <- "Adenosine dosage (µg/kg/min)"
label(stress$peaksbp) <- "Peak systolic BP (mm Hg)"
label(stress$peakhr) <- "Peak HR (bpm)"
label(stress$resthr) <- "Rest HR (bpm)"
label(stress$hr_delta) <- "Change in HR (bpm)"
label(stress$dbp_delta) <- "Change in diastolic BP (mm Hg)"
label(stress$sbp_delta) <- "Change in systolic BP (mm Hg)"
label(stress$rpp) <- "Rest rate-pressure product"
label(stress$srpp) <- "Stress rate-pressure product"
label(stress$ptt_centroid) <- "Pulmonary transit time, centroid"
label(stress$ptt_ptp) <- "Pulmonary transit time, peak to peak"

stress$cav <- data$cav
stress %>% select(-c(age,bsa,stressor,lvef,gls,nosymptom,time_from_tx,time_angio_tx,time_cav_cmr, mace_time, cav_angio,cav_ctca,cav_grade,rejection,death,hosp,mace,tx_old,rmbf, gender, weight, dsa, dsa_fup, mbf, mpr, ischemic_burden)) %>% mutate(
  peaksbp=as.numeric(peaksbp),
  peakhr=as.numeric(peakhr),
  resthr=as.numeric(resthr),
  srpp=as.numeric(srpp),
  rpp=as.numeric(rpp),
  #ischemic_burden=as.numeric(ischemic_burden),
  hr_delta=as.numeric(hr_delta),
  dbp_delta=as.numeric(dbp_delta),
  sbp_delta=as.numeric(sbp_delta),
  rpp=as.numeric(rpp)) %>% 
  tbl_summary(by = cav, missing = "no",
  digits=list(
    c(mbfi, mpri, rmbfi) ~ c(1,2)
  ),
  label=list(
    ado_dose  ~ "Adenosine dosage (µg/kg/min)",
    peaksbp  ~ "Peak systolic BP (mm Hg)",
    peakhr  ~ "Peak HR (bpm)",
    resthr  ~ "Rest HR (bpm)",
    hr_delta  ~ "Change in HR (bpm)",
    dbp_delta  ~ "Change in diastolic BP (mm Hg)", 
    sbp_delta  ~ "Change in systolic BP (mm Hg)",
    rpp  ~ "Rest rate-pressure product", 
    #ischemic_burden ~ "Ischemic burden (%)",
    srpp  ~ "Stress rate-pressure product", 
    ptt_centroid  ~ "Pulmonary transit time, centroid", 
    ptt_ptp  ~ "Pulmonary transit time, peak to peak"
  )) %>% add_overall() %>% add_p() %>% as_gt()
...
::: {#fig-qp-cav}
![](images/phd/qp-cav.png)

Stress MBF  (mL/min/g) and MPR index in patients with and without significant CAV (ISHLT grade 2-3).
:::
...
Comprehensive results are presented in @tbl-acc. Visual assessment and a strategy based on MPR index were compared for reclassification across the range of pre-test probability of CAV (@fig-leaf). A quantitative approach based exclusively on the measurement of stress MPR index, where CAV would be adjudicated for MPR index ≥ 2.1, was effective at ruling-in and ruling-out CAV, while qualitative assessment of ischemia offered almost non-existent ruling-out capacity.
...
Pre- and post-test probability of CAV based on stress CMR results, visual assessment and quantitative assessment with MPR index compared.
:::
...
![Net reduction in unnecessary angiography](images/phd/dca-netred.png)
...
```{r dca-tbl}
#| tbl-cap: Net benefit of undergoing QP-CMR before any angiography decision {#tbl-netben}
library(dplyr);library(echarts4r);library(tidyr)

data.frame(
  "ptp" = as.character(c(10, 15, 20, 25, 30, 35, 40)),
  "angio" = c(16, 11, 6, -1, -8, -16, -26),
  "none" = c(0, 0, 0, 0, 0, 0, 0),
  "QP" = c(16, 12, 11, 9, 7, 5, 2),
  "ischemia" = c(16, 11, 6, 4, 3, 2, 1)
) %>%
  pivot_longer(c(angio, none, QP, ischemia)) %>%
  mutate(name = case_when(
    name == "angio" ~ "CA\n\nto all",
    name == "none" ~ "No testing",
    name == "QP" ~ "MPRi\n\nguided CA",
    name == "ischemia" ~ "Ischemia\n\nguided CA"
  )) %>%  
    e_charts(name) %>% 
    e_heatmap(ptp, value) %>% 
        e_visual_map(value,
        inRange = list(color = c("#202e32", "#586c5c","#dfdcb9"))
        ) %>% e_labels(position="inside") %>% e_theme("vintage") %>% e_axis_labels(x="Strategy", y ="Threshold probability")
```


```{r netred}
#| tbl-cap: Net reduction in angiographies per 100 patients. 
# Table 2: Net Benefit of QP
data.frame(
  "ptp" = as.character(c(10, 15, 20, 25, 30, 35, 40)),
  "angio" = c(0, 0, 0, 0, 0, 0, 0),
  "none" = c(0, 0, 0, 2, 19, 30, 39),
  "QP" = c(0, 5, 20, 29, 35, 39, 42),
  "ischemia" = c(0, 0, 0, 13, 25, 34, 40)
) %>% pivot_longer(c(angio, none, QP, ischemia)) %>%
  mutate(name = case_when(
    name == "angio" ~ "CA\n\nto all",
    name == "none" ~ "No testing",
    ...
    e_charts(name) %>% 
    e_heatmap(ptp, value) %>% e_visual_map(value, 
    inRange = list(color = c("#202e32", "#586c5c","#dfdcb9"))
    ) %>% e_labels(position="inside") %>% e_theme("vintage") %>% e_legend(FALSE) %>% 
    e_x_axis(name="Strategy", axisLabel=list(position="top")) %>% 
    e_y_axis(name="Net reduction\n per 100 angiographies", axisLabel=list(position="top"))
```
...
Among stress CMR parameters, stress MBFi and MPRi, but not qualitative assessment of ischemia, were prognostic (Table 5). Multivariable adjustment was not attempted due to the low sample size. Exploratory plotting of hazard ratios on the spectrum of stress MPR index values and stratification by MPR index for Kaplan Meier curves demonstrated that patients with an MPR index lower than 2.0 were at significantly increased risk of future MACE (@fig-km and @suppfig-mace).

::: {#fig-km}
![](images/phd/qp-km.png)

Prognostic value of MPR index among outpatient heart transplant recipients.
:::
...
::: {#suppfig-dca}
![](images/phd/qp-dca-crossval.png)

10-fold cross-validated prediction model for net benefit.
:::

::: {#suppfig-mace}
![](images/phd/mpri-mace.png)

Hazard ratio for MACE as a function of MPR index.
:::

::: {#suppfig-rrpp}
![](images/phd/rmbf-rrpp.png)

Relationship between rest RPP and rest MBF.
:::

::: {#suppfig-srpp}
![](images/phd/smbf-srpp.png)

Relationship between stress RPP and stress MBF.
:::

::: {#suppfig-rpp}
![](images/phd/rpp-stress.png)

The rate-pressure product at rest and stress.
:::

::: {#suppfig-rpp-sex}
![](images/phd/rpp-sex.png)

Sex-specific change in rate-pressure product between rest and stress.
:::

::: {#suppfig-rpp-dsa}
![](images/phd/rpp-dsa.png)

Change in rate-pressure product between rest and stress in patients with and without DSA.
:::

::: {#suppfig-smbf-age}
![](images/phd/smbf-age.png)

Stress MBF index and graft age.
:::
