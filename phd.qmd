...
```{r}
stress$cav <- data$cav
stress %>% select(-c(age,bsa,stressor,lvef,gls,nosymptom,time_from_tx,time_angio_tx,time_cav_cmr, mace_time, cav_angio,cav_ctca,cav_grade,rejection,death,hosp,mace,tx_old,rmbf, gender, weight, dsa, dsa_fup)) %>% mutate(
  hr_delta=as.numeric(hr_delta),
  dbp_delta=as.numeric(dbp_delta),
  sbp_delta=as.numeric(sbp_delta),
  rpp=as.numeric(rpp)) %>% 
  tbl_summary(by = cav, missing = "no") %>% add_overall() %>% add_p() %>% as_gt()
```
:::
